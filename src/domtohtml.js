...
exports.stringifyElement = function stringifyElement(element) {
		// process.stdout.write('Stringify HTML for: ' + element + "\n");
...
exports.generateHtmlRecursive = function generateHtmlRecursive(node, rawText,nrecurse,tags) {
	...
	if (node) {
		...
		switch (node.nodeType) {
			case node.ELEMENT_NODE:
				...
				if(tags) ret += current.start;
				
				if (node._childNodes.length > 0) {
					for (i=0; i<node._childNodes.length; i++) {
							ret += exports.generateHtmlRecursive(node._childNodes[i], rawText,nrecurse,tags);
					}
				} else {
					//ret += rawText ? node.nodeValue : HTMLEncode(node.nodeValue);
					ret += node.nodeValue || '';
				}
				if(tags) ret += current.end;
				break;
			case node.TEXT_NODE:
				//ret += rawText ? node.nodeValue : HTMLEncode(node.nodeValue);
				var clean = node.nodeValue.trim().replace(/\n/g," ") + " ";
				ret += clean
				break; 
...
exports.domToHtml = function(dom,nrecurse,tags) {
	var ret = "";
	if (dom.toArray) {
		// node list
		dom = dom.toArray();
	...
	if (Array.isArray(dom)) {
		for (var i=0,len=dom.length; i<len; i++) {
			ret += exports.generateHtmlRecursive(dom[i],false,nrecurse,tags);
		}
	} else {
		// single node
